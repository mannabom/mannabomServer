...
public class RedisMatchAtomicOps {
    ...
    //동시성 문제 해결-> 여러 큐를 순회하면서 특정 유저 지우기
    private static final DefaultRedisScript<Long> REMOVE_SCRIPT = new DefaultRedisScript<>(
            ...
            "if rm >0 then return 1 else return 0 end", Long.class
            );

    public boolean tryLock(Long meetingId){
        String key = lockKey(meetingId);
        return Boolean.TRUE.equals(
                redis.opsForValue().setIfAbsent(key,"1",LOCK_TTL)
        );
    }
    public void tryUnlock(Long meetingId){
        redis.delete(lockKey(meetingId));
    }
    ...
    private String lockKey(Long meetingId){
        return String.format("lock:match:"+meetingId);
    }
}
