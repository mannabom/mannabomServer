...
public class PhotoRequestController {
    ...
    public ResponseEntity<ApiResponse<LoveViewPhotoStatusResponse>> getProfileStatus(@PathVariable Long chatRoomId, @AuthenticationPrincipal Long userId){
        ...
        switch(status){
            case PENDING -> systemMessage = "서로를 알아가는 단계예요. 대화를 조금 더 나눠보세요! (10마디 미만)";
            case REJECTED -> systemMessage ="서로 충분히 대화했네요! 이제 상대방에게 프로필 사진을 요청해 볼까요?";
            case READY -> systemMessage = "상대방의 수락을 기다리고 있어요. 조금만 기다려 주세요!";
            case ACCEPTED -> systemMessage = "상대방이 프로필 사진 공개를 요청했어요! 수락하시겠어요?";
            case WAITING -> systemMessage = "서로의 프로필이 공개되었습니다. 즐거운 대화 나누세요! ✨";
            case RECEIVED -> systemMessage ="상대방이 아직은 조심스러운가 봐요. 10마디 더 나누면 다시 요청할 수 있어요.";
        }
